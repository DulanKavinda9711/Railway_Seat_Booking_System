...
namespace TrainUser
{
    public partial class Form1 : Form
    {
        ...
        {
            // Assuming lb1 to lb20 are the labels whose color needs to be changed
            ...
            // If textBoxNum is empty, set all label colors to default and return
            if (string.IsNullOrEmpty(textBoxNumText))
            {
                foreach (var label in labels)
                {
                    label.BackColor = Color.Green; // Assuming green color indicates available seats
                }
                return;
            }
            ...
            // Update label colors based on booked seats
            foreach (string seatSegment in bookedSeats)
            {
                string trimmedSegment = seatSegment.Trim(); // Trim leading and trailing spaces
                int seatNumber;
                if (int.TryParse(trimmedSegment, out seatNumber) && seatNumber >= 1 && seatNumber <= 20)
                {
                    labels[seatNumber - 1].BackColor = Color.Red; // Assuming red color indicates booked seats
                }
                else
                {
                    // Split the segment further by non-digit characters to handle multiple seat numbers in one segment
